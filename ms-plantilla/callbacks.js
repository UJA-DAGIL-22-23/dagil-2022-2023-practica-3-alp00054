...
const client = new faunadb.Client({
    secret: 'fnAFAslydEAAzZYez5qU6D4McFsXXAydppvIUxh7',
});

const COLLECTION = "Pilotos"
...
const CB_OTHERS = {
    ...
    acercaDe: async (req, res) => {
        try {
            CORS(res).status(200).json({
                ...
                autor: "Ana León Pulido",
                email: "alp00054@red.ujaen.es",
                fecha: "18-04-2023"
            });
        } catch (error) {
            CORS(res).status(500).json({ error: error.description })
    ...
    /**
     * Método para obtener todos los pilotos de la BBDD.
     * @param {*} req Objeto con los parámetros que se han pasado en la llamada a esta URL 
     * @param {*} res Objeto Response con las respuesta que se va a dar a la petición recibida
     */
    get_pilotos: async (req, res) => {
        try {
            let personas = await client.query(
                q.Map(
                    q.Paginate(q.Documents(q.Collection(COLLECTION))),
            ...
            // console.log( personas ) // Para comprobar qué se ha devuelto en pilotos
            CORS(res)
                .status(200)
                .json(personas)
        } catch (error) {
    ...
    /**
     * Método para obtener todos los datos de los pilotos de la BBDD.
     * @param {*} req Objeto con los parámetros que se han pasado en la llamada a esta URL 
     * @param {*} res Objeto Response con las respuesta que se va a dar a la petición recibida
     */
    get_pilotos_completos: async (req, res) => {
        try {
            let personas = await client.query(
                q.Map(
                    q.Paginate(q.Documents(q.Collection(COLLECTION))),
    ...
    /**
     * Método para obtener todos los nombres de los pilotos ordenados de la BBDD.
     * @param {*} req Objeto con los parámetros que se han pasado en la llamada a esta URL 
     * @param {*} res Objeto Response con las respuesta que se va a dar a la petición recibida
     */
    get_pilotos_ordenados: async (req, res) => {
        try {
            let personas = await client.query(
                q.Map(
                    q.Paginate(q.Documents(q.Collection(COLLECTION))),
